...
namespace AI_Answer {
  ...
  class array {
   ...
   public:
    ...
    constexpr T at(const std::uint8_t i) noexcept { return sequence[i]; }

    constexpr T operator[](const std::uint8_t i) noexcept { return sequence[i]; }
    ...
    {
      for(int i : std::initializer_list<int>{ args... }) probs[i] += 1.0 / size();
    }
  };
}  // namespace AI_Answer
