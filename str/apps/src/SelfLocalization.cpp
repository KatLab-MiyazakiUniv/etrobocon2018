...
bool SelfLocalization::approached_target_coordinates(float target_x, float target_y,
                                                     float target_radius)
{
  float distance = std::sqrt((target_x - current_x) * (target_x - current_x)
                             + (target_y - current_y) * (target_y - current_y));
  if(distance < target_radius) return true;
  return false;
}
...
//式の整理は自分で計算したやつ
float SelfLocalization::calculate_between_ev3_and_border(float _start_x, float _start_y,
                                                         float _goal_x, float _goal_y,
...
void SelfLocalization::init_normal_vector(float _start_x, float _start_y, float _goal_x,
...
bool SelfLocalization::is_over_normal_vector(float _current_x, float _current_y)
{
