...
void BasicWalker::spin(bool rotationalDirection, int32_t angle, int32_t turn)
{
  ...
  while((walker.get_count_R() * reverseValue)
    ...
    walker.run(0, (turn * reverseValue));
...
void BasicWalker::goStraightToCheckBlackLine(int32_t target_forward, int32_t distance,
