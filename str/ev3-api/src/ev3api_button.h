 ...
 * \details       When an invalid button number is specified , always returns \a false  [TODO: check
 * - error log is output]． \param  button Button number \retval true   Pressed state \retval false
 * Unpressed state
 *
 * \~Japanese
 * \brief         ボタンの押下状態を取得する．
 ...
 * \details		  The button handler executes in the task contest. The default is called from the wait
 * disabled state. \param button  the button to be set \param handler the handler to be attached,
 * NULL for clearing the current handler \param exinf   extra information passed to the \a handler
 * when it is called \retval E_OK   Successful completion \retval E_ID   Invalid button number
 *
 * \~Japanese
