 ...
 * \brief           Get font size.
 * \param  font     Font number
 * \param  p_width  Pointer to a place to store the acquired font width. If it is NULL, it is not
 * stored. \param  p_height Pointer to the place to store the height of the acquired font. If it is
 * NULL, it is not stored. \retval E_OK     Successful completion \retval E_ID     Invalid fomt
 * number
 *
 * \~Japanese
 * \brief           フォントのサイズを取得する．
 ...
 * \details          Create an image object from the specified memory file. Currently, it only
 * supports monochrome image files in BMP format. If an error occurs , clear the data of \a p_image
 * to NULL. \param  p_memfile Object of memory file where image file is stored \param  p_image
 * Pointer to the place to store the object of the generated image \retval E_OK      Successful
 * completion \retval E_NOMEM   Insufficient memory \retval E_NOSPT   Image file format is not
 * supported \retval E_OBJ     Image file is damaged \retval E_PAR     Invalid memory file
 *
 * \~Japanese
 * \brief            メモリファイルから画像をロードする．
 * \details
 * 指定したメモリファイルから画像のオブジェクトを生成する．現時点では，BMP形式のモノクロ画像ファイルしかサポートしない．
 ...
 * \details          Free resources (memory area) secured by the object of the image. In case of
 * normal \a termination , data of \a p_image is cleared to NULL. \param  p_image   Pointer to
 * released object \retval E_OK      Successful completion \retval E_PAR     \a p_image is NULL
 *
 * \~Japanese
 * \brief            画像のオブジェクトを解放する．
 * \details 画像のオブジェクトにより確保されたリソース（メモリ領域）を解放する．正常終了の場合，\a
 * p_image の \a data はNULLにクリアする． \param  p_image   解放する画像のオブジェクトのポインタ
 * \retval E_OK      正常終了
