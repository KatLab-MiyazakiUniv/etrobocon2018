 ...
 * \details	   Set the type of motor connected to the motor port. You can also specify a new motor
 * type even if you have already set it. \param port Motor port to be configured \param type Motor
 * type for the specified motor port \retval E_OK  Successful completion \retval E_ID  Illegal motor
 * port number \retval E_PAR Illegal motor type
 *
 * \~Japanese
 * \brief 	     モータポートを設定する．
 * \details
 ...
 * \details    Setting the value of the angular position sensor of the motor does not affect the
 * actual power and position of the motor. \param port Motor port to be inquired \retval E_OK
 * Successful completion \retval E_ID   Illegal motor port number \retval E_OBJ  Motor port has not
 * been initialized.
 *
 * \~Japanese
 ...
 * \param power The percentage of full power, ranging from -100 to +100. A negative value makes the
 * motor rotate backwards. \retval E_OK     Successful completion \retval E_ID     Illegal motor
 * port number \retval E_OBJ    Motor port has not been initialized.
 *
 * \~Japanese
 * \brief 	     モータのパワーを設定する
 * \param  port  モータポート番号
 * \param  power
 * モータのフルパワーのパーセント値．範囲：-100から+100．マイナスの値でモータを逆方向に回転させることができる．
 * \retval E_OK  正常終了
 * \retval E_ID  不正のモータポート番号
 * \retval E_OBJ モータ未接続
 */
 ...
 * \param speed_abs Speed for rotating. The value is a percentage of full speed, ranging from 0 to
 * +100. \param blocking  \a true (The function will be blocked until the rotation is finished), or
 * \a false (The function will not be blocked). \retval E_OK     Successful completion \retval E_ID
 * Illegal motor port number \retval E_OBJ    Motor port has not been initialized.
 *
 * \~Japanese
 ...
 * \param  blocking  \a true (関数は回転が完了してからリターン)，\a false
 * (関数は回転操作を待たずにリターン) \retval E_OK      正常終了 \retval E_ID 不正のモータポート番号
 * \retval E_OBJ     モータ未接続
 */
 ...
 * \param  power       Power of motors. Range: -100 to +100. A negative value moves the robot
 * backwards. \param  turn_ratio  The sharpness of the turn. Range: -100 to +100. If \a turn_ratio
 * is negative, the robot will turn left. If \a turn_ratio is positive, the robot will turn right.
 * More specifically, \a turn_ratio determines the ratio of inner wheel speed as a percent. For
 * example, if \a turn_ratio is +25, the right motor will move at 75% of the \a power, which makes
 * the robot turn right. \retval E_OK        Success \retval E_ID        Invalid ID of motor port
 * \retval E_OBJ       Motor port has not been initialized.
 *
 * \~Japanese
 ...
 * \param  turn_ratio
 * ステアリングの度合い．範囲：-100から+100．マイナスの値は左への転回，プラスの値は右への転回になる．
 *                     具体的に言えば，このパラメータはこの左右モータのパワーの差の度合いである．例えば，\a
 * turn_ratio は+25である場合， 左モータのパワーは\a power で，右モータのパワーは\a power
 * の75\%になり，ロボットは右へ転回する． \retval E_OK        正常終了 \retval E_ID 不正のモータID
 * \retval E_OBJ       モータ未接続
 */
