 ...
 * \param  volume  The percentage of max volume level. Range: 0 to +100. 0 means mute. If an
 * out-of-range value is given, i.e. larger than 100, it will be clipped to 100, the maximum value.
 * \retval E_OK    Success
 ...
 * \brief          音量を調整する．
 * \param  volume
 * ボリュームの値．範囲：0から+100．0はミュート．+100を超えた値を指定すると，実際の値は+100になる．
 * \retval E_OK    正常終了
...
#define SOUND_MANUAL_STOP \
  (-1)  //!< \~English Flag for stopping sound reproduction.   \~Japanese
        //!< 音声の再生を手動で停止するためのフラグ

/**
 * \~English
 * \brief           Play a tone. Any sound being played will be stopped by calling this function.
 * \param frequency Frequency of the note, in Hertz (Hz). Range: 250 - 10000. If a out-of-range
 * value is given, it will be clipped to the minimum or maximum value. \param duration  Duration to
 * play, in milliseconds (ms). If a negative value is given, it will keep playing until stopped
 * manually. If 0 is given, it will just stop the sound being played. \retval E_OK     Success. The
 * note is now being played. \retval E_CTX    Not called from task context. \retval E_NORES  Failed
 * to acquire the sound device.
 *
 * \~Japanese
...
/**
 * \~English
 * \brief            Play a WAV file stored in memory. Only 8-bit 8kHz mono WAV file is supported by
 * now. Any sound being played will be stopped by calling this function. \param  p_memfile Pointer
 * of a memory file which holds the content of the WAV file to be played. \param  duration  Duration
 * to play, in milliseconds (ms). If SOUND_MANUAL_STOP is specified,it will keep playing until
 * stopped manually or finished. If 0 is given, it will just stop the sound being played. \retval
 * E_OK      Success. The WAV file is now being played. \retval E_CTX     Not called from task
 * context. \retval E_PAR     Not a valid or supported WAV file. \retval E_NORES   Failed to acquire
 * the sound device.
 *
 * \~Japanese
 * \brief            指定したWAVファイル（8-bit 8kHz
 * mono）を再生する．今再生しているサウンドは停止される． \param  p_memfile
 * WAVファイルのメモリファイルへのポインタ \param  duration
 * 出力持続時間．単位：ミリ秒．SOUND_MANUAL_STOPを指定した場合は手動で停止しないと最後まで再生する．
 * \retval E_OK      正常終了
 * \retval E_CTX     非タスクコンテストから呼び出す
 * \retval E_NORES   サウンドデバイスが占有されている
 */
...
#define NOTE_CS6 \
  (1108.73)  //!< \~English Frequency of musical note C#6 \~Japanese ノートC#6の周波数
#define NOTE_D6 (1174.66)  //!< \~English Frequency of musical note D6  \~Japanese ノートD6の周波数
#define NOTE_DS6 \
  (1244.51)  //!< \~English Frequency of musical note D#6 \~Japanese ノートD#6の周波数
...
#define NOTE_FS6 \
  (1479.98)  //!< \~English Frequency of musical note F#6 \~Japanese ノートF#6の周波数
#define NOTE_G6 (1567.98)  //!< \~English Frequency of musical note G6  \~Japanese ノートG6の周波数
#define NOTE_GS6 \
  (1661.22)  //!< \~English Frequency of musical note G#6 \~Japanese ノートG#6の周波数
#define NOTE_A6 (1760.00)  //!< \~English Frequency of musical note A6  \~Japanese ノートA6の周波数
#define NOTE_AS6 \
  (1864.66)  //!< \~English Frequency of musical note A#6 \~Japanese ノートA#6の周波数
