...
typedef enum {
  ...
  HT_NXT_ACCEL_SENSOR,  //!< \~English HiTechnic NXT acceleration sensor \~Japanese
                        //!< 加速度センサ（HiTechnic社製）
  HT_NXT_COLOR_SENSOR,  //!< \~English HiTechnic NXT color sensor        \~Japanese
                        //!< カラーセンサ（HiTechnic社製）
 ...
 * \details    Always return COLOR_NONE (error  is outputted) when an incorrect sensor number is
 * specified. \param port Sensor port to be inquired \return     Color detected
 *
 * \~Japanese
 ...
 * \details      When an invalid sensor support number is specified, always returns false (error log
 * is output). \param port   Sensor port to be inquired \retval true  A signal has been received
 * \retval false No signal has been received
 *
 * \~Japanese
 * \brief 	     超音波センサで超音波信号を検出する．
 * \details      不正のセンサポート番号を指定した場合，常に \a false
 * を返す（エラーログが出力される）． \param  port  センサポート番号 \retval true
 * 超音波信号を検出した \retval false 超音波信号を検出しなかった
 */
 ...
 * \details    When an invalid sensor support number is specified, always return 0 direction and
 * distance of -128 (error log is output). \param port Sensor port to be inquired. \return Struct
 * with heading/distance for all (4) channels.
 *
 * \~Japanese
 ...
 * \details    When an invalid sensor support number is specified, always return 0 pattern (error
 * log is output). \param port Sensor port to be inquired. \return     Struct with details of the IR
 ...
 * \details      When an invalid sensor support number is specified, always returns false (error log
 * is output) \param port   Sensor port to be inquired \retval true  Pressed  pressed \retval false
 * Unpressed pressed
 *
 * \~Japanese
 * \brief 	     タッチセンサの状態を検出する．
 * \details      不正のセンサポート番号を指定した場合，常に \a false
 * を返す（エラーログが出力される）． \param port   センサポート番号 \retval true  押されている状態
 ...
 * \details      When an invalid sensor support number is specified, always returns false (error log
 * is output) \param port   Sensor port to be inquired \param axes   Array to store the x/y/z axes
 * data \retval true  Axes [] is  updated \retval false Axes[] is unchanged due to I2C busy
 *
 * \~Japanese
 * \brief 	     加速度センサ（HiTechnic社製）で加速度を測定する．
 * \details      不正のセンサポート番号を指定した場合，常に \a false
 * を返す（エラーログが出力される）． \param  port  センサポート番号 \param  axes
 * x軸、y軸、z軸の3方向の加速度を格納するための配列 \retval true  axes[]は更新された \retval false
 * axes[]は変更されなかった（前回のI2C操作が完成していない）
 */
 ...
 * \details      When an invalid sensor support number is specified, always returns false (error log
 * is output) \param port   Sensor port to be inquired \param color  Pointer to store color number
 * (0-17) \retval true  color is updated \retval false color is unchanged due to I2C busy
 *
 * \~Japanese
 * \brief        カラーセンサ（HiTechnic社製）でカラーを測定する．
 * \details      不正のセンサポート番号を指定した場合，常に \a false
 * を返す（エラーログが出力される）． \param  port  センサポート番号 \param  color
 * カラーの番号（0-17）を格納するためのポインタ \retval true  colorは更新された \retval false
 * colorは変更されなかった（前回のI2C操作が完成していない）
 */
 ...
 * \details      When an invalid sensor support number is specified, always returns false (error log
 * is output) \param port   Sensor port to be inquired \param val    Pointer to store RGB raw value
 ...
 * \details      不正のセンサポート番号を指定した場合，常に \a false
 * を返す（エラーログが出力される）． \param  port  センサポート番号 \param  val   RGB
 * Raw値を格納するためのポインタ \retval true  valは更新された \retval false
 * valは変更されなかった（前回のI2C操作が完成していない）
 */
 ...
 * \details      When an invalid sensor support number is specified, always returns false (error log
 * is output) \param port   Sensor port to be inquired \param temp   Variable to store the
 * temperature value \retval true  Temp is updated) \retval false Temp is unchanged due to I2C busy
 *
 * \~Japanese
 * \brief 	     NXT温度センサ（9749）で温度を測定する．
 * \details      不正のセンサポート番号を指定した場合，常に \a false
 * を返す（エラーログが出力される）． \param  port  センサポート番号 \param  temp
 * 温度データ（°C）を格納するための変数へのポインタ \retval true  tempは更新された \retval false
 * tempは変更されなかった（前回のI2C操作が完成していない）
 */
