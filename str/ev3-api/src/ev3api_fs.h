 ...
 * \details  This page describes only API unique to this platform, but it also supports file
 * manipulation functions of standard C libraries such as fopen ().
 * @{
 *
 * \~Japanese
 ...
 */
typedef struct {
  void* buffer;  //!< \~English Pointer of the buffer where this file is stored. NULL means a
                 //!< invalid memory file. \~Japanese
                 //!< このファイルが格納されているバッファへのポインタ．NULLは無効なメモリファイル．
  uint32_t filesz;  //!< \~English Actual size of this file \~Japanese ファイルの実際のサイズ
  uint32_t buffersz;  //!< \~English Maximum size of the buffer to store this file
                      //!< \~Japanese バッファの最大サイズ
} memfile_t;

#define TMAX_FILENAME_LEN \
  (255)  //!< \~English Maximum length of a file name \~Japanese ファイル名の最大の長さ
...
typedef struct {
  ...
  bool_t
      is_readonly;  //!< \~English Flag of read-only  \~Japanese 読み出し専用であることを表すフラグ
 ...
 * \details    If it succeeds, it returns the ID of the opened directory as a return value. This ID
 * can be used to obtain file information in the directory. \param path Path of the directory to be
 * opened \retval >0     ID of the opened directory \retval E_CTX  Call from non-task contest
 ...
 * \details           If it succeeds, it releases the resource of the opened directory, and its ID
 * can not be used. \param  dirid      ID of an opened directory \retval E_OK       Success \retval
 * E_CTX      Call from non-task contest \retval E_ID       Invalid ID number \retval E_SYS      I/O
 * error occurred (High possibility of SD card failure)
 *
 * \~Japanese
 * \brief          ディレクトリをクローズする．
 ...
 * \details          Generates an object of the memory file and reads the specified file from the SD
 * card into this memory file. If an error occurs , clear buffer in \a p_memfile to \a NULL. \param
 * path      Path of the file to be loaded \param  p_memfile Pointer of save the created memory file
 * \retval E_OK      Success
 * \retval E_MACV    Memory access violation (path or p_memfile)
 * \retval E_NOMEM   No enough free memory to create the memory file, or \a p_memfile is NULL. The
 * \a buffer of \a p_memfile will be set to NULL if this happens. \retval E_PAR     The \a path does
 * not point to a valid file. The \a buffer of \a p_memfile will be set to NULL if this happens.
 * \retval E_SYS     I/O failure, which might be caused by a corrupted SD card. The \a buffer of \a
 * p_memfile will be set to NULL if this happens.
 *
 * \~Japanese
 * \brief            SDカードのファイルをメモリファイルとしてロードする．
 * \details
 * メモリファイルのオブジェクトを生成して，指定したファイルをSDカードからこのメモリファイルに読み込む．エラーが発生する場合，\a
 * p_memfile の \a buffer はNULLにクリアする. \param  path      ファイルのパス \param  p_memfile
 * 生成したメモリファイルのオブジェクトを格納する場所へのポインタ \retval E_OK      正常終了 \retval
 * E_MACV    メモリアクセス違反（pathかp_memfile） \retval E_NOMEM   メモリ不足 \retval E_PAR
 * パス名は無効 \retval E_SYS     I/Oエラーが発生した（SDカード不良の可能性が高い）
 */
 ...
 * \retval E_OK      Success
 * \retval E_PAR     The \a p_memfile is NULL. [TODO: check - inconsistent with Jp version ->
 * E_MACV] \retval E_OBJ     The \a p_memfile does not point to a valid memory file.
 *
 * \~Japanese
 * \brief            メモリファイルを解放する．
 * \details メモリファイルにより確保されたリソース（メモリ領域）を解放する．正常終了の場合，\a
 * p_memfile の \a buffer はNULLにクリアする． \param  p_memfile 解放するメモリファイルのポインタ
 * \retval E_OK      正常終了
 ...
 */
typedef enum {
  EV3_SERIAL_DEFAULT = 0,  //!< \~English Default serial port (port for log task)     \~Japanese
